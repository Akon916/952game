...
$(function(){
    var inp1=false,inp2=false,inp3=false,inp4=false,inp5=false,inp6=false,inp7=false,inp8=false,inp9=false;
    // 验证用户名
    ...
    $('#id_code').focus(function() {
        $(this).parent().next().text("请输入验证码").css('color', '#000000');
    }).blur(function() {
        if ($('#id_code').val() !== '') {
            $(this).parent().next().next().text('* 已输入').css('color','green');
            inp9 = true;
        } else {
             $(this).parent().next().next().text('* 验证码不能为空').css('color','red');
            inp9 = false;
        }
    });
    if ($('#id_code').val() == '') {
    ...
    $('#submit').click(function(){
        if(inp1 && inp2 && inp3 && inp4 && inp5 && inp6 && inp7 &&inp8 && inp9){
            $('form').submit();
        }else{
            return false;
        }
